...
def create_dataframe(data: dict) -> pd.DataFrame:
    ...
    """
    try:
        #[3,4,11,22,115] is list of non-far logical entities as per old color code scheme, will be updated with new color code scheme.
...
def generate_gc_breakup_report(df, non_far_colors = [4,3,115,22,11], floor_layer='FLOOR-GROUND'):
    ...
    totals = {
        "Plinth Area": 0,
        "Plinth Deductible Area": 0,
        "Outer Allowable Area": 0,
        "Outer Non Allowable Area": 0
    }
    ...
    if not plinth_polygon_df.empty:
        ...
        totals['Plinth Area'] = round(plinth_area,3)
    else:
        plinth_area = 0
    report_data.append({
                    "Floor Name": floor_layer,
                    "Plinth": plinth_value,
    ...
    for idx, non_far_row in non_far_df.iterrows():
        ...
        if not plinth_polygon_df.empty:
            ...
            if plinth_polygon.contains(non_far_polygon):
                ...
                report_data.append({
                    "Floor Name": floor_layer,
                    "Object in Plinth": non_far_text,
                    "Object outside Plinth": "",
                    "Plinth Deductible Area": round(non_far_area,3),
                    "Outer Allowable Area": "",
                    "Outer Non Allowable Area": ""
                })
                totals['Plinth Deductible Area'] += non_far_area
            else:
                # Object Outside Plinth
                if non_far_area < 25:
                ...
                report_data.append({
                    "Floor Name": floor_layer,
                    "Object in Plinth": "",
                    "Object outside Plinth": non_far_text,
                    "Plinth Deductible Area": "",
                    "Outer Allowable Area": allowable_area,
                    "Outer Non Allowable Area": non_allowable_area
                })
                totals['Outer Allowable Area'] += allowable_area
                totals['Outer Non Allowable Area'] += non_allowable_area

    # Final Report Structure
    coverage_area = totals["Plinth Area"] - totals["Plinth Deductible Area"] + totals['Outer Non Allowable Area'] 
    coverage_perc = round(coverage_area/plot_area,2)*100
    report_json = {
        "data": report_data,
        "totals": totals,
        "Coverage Area": coverage_area,
        "Existing Coverage": None,  # Assuming 0 for this case
        "Coverage Percentage": coverage_perc
    }
...
def count_escalators(df):
    """
    This function counts the number of escalators.
    """
...
def count_moving_walks(df):
    """
    This function counts the number of moving walks.
    """
        ...
        for index, building_row in df_building.iterrows():
            ...
            for i in range(len(building_coords) - 1):
                ...
                if balcony_lengths:
                    ...
                    result.append({
                        "blockName": "Single",  # Keeping specfic block name, will be adjusted once Flatted development is done.
                        "floorName": layer,
                        "buildingLength": round(building_length, 2),
                        "balconyLengths": sorted_balcony_lengths,  
...
def get_habitable_rooms(df, color_code, kitchen_area):
    ...
    for i, room in habitable_room_df.iterrows():
        ...
        habitable_rooms_data.append({
            'Room Name': f'Habitable Room {habitable_count}', 
...
def calculate_floor_wise_breakup(df):
    ...
    for floor in floor_types:
        ...
        for idx, wc_data in enumerate(combined_wc_data, start=1):
            floor_result['Combined Bathroom and WC'].append({
                'Name': f'Combined Bathroom and WC {idx}',
                'Area': wc_data['Area'],
                'Width': wc_data['Width']
            })
        ...
        floor_result['Kitchen'] = {'Area': kitchen_area, 'Width': kitchen_width}

        # Habitable Rooms
        habitable_rooms_data = get_habitable_rooms(floor_df, color_codes['Habitable Room'], kitchen_area)
        floor_result['Habitable Room'] = habitable_rooms_data
        ...
        floor_result['Min Ventilation'] = min_ventilation_compliance

        floors[floor] = floor_result
    return floors
...
# Extract data
    ...
    'Staircase Width': df_width.to_dict(),
    'Staircase Tread and Riser': df_tread_riser.to_dict(),
    'Handrail Height': df_handrail.to_dict(),
    'Riser Count per Flight': df_riser_count,
    'landing_widths':landing_widths
    }

    #output_json = json.dumps(stair_details_output, indent=4)
...
def calculate_watertanka_volume(df):
    ...
    for index, row in df_watertank.iterrows():
        ...
        height_texts = [text for text in row['Text'] if 'Height' in text]
        
        if height_texts:
            # Use regular expression to extract the numeric height value
            height_match = re.search(r'Height\s*-\s*([\d.]+)', height_texts[0])
            if height_match:
...
def building_general_details(df):
    ...
    building_details = [
        {
            "Block Name": "Single",
            "Total Number of Floors": f"{total_floors} Floors",
            "Block Height": block_height,
            "Total Builtup Area": round(total_builtup_area, 3),
            "Non FAR Area": round(non_far_area, 3),
    ...
    building_details_json = json.dumps(building_details, indent=4)
...
def create_non_far_json_table(df):
    ...
    for block in blocks:
        ...
        for floor in floors:
            ...
            floor_data = {
                'Typical Floor Name': floor,
                'Non FAR Objects': [
                    {'Non FAR Name': name, 'Non FAR Area': round(area,3)} for name, area in non_far_objects
                ],
                'Total Non FAR Area': round(calculate_total(non_far_objects), 3)
            }
...
def floor_wise_breakup_json(df):
    ...
    for floor_name in filtered_floors:
        ...
        floors.append({
            ...
            "Not Counted in FAR Area": round(non_far_area, 3),
